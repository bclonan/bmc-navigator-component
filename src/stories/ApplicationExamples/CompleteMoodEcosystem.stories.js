...
import AdvancedMoodAnalytics from '../../components/forms/AdvancedMoodAnalytics.vue';
import EmotionDrivenInterface from '../../components/forms/EmotionDrivenInterface.vue';
import LoanApplicationForm from '../../components/forms/LoanApplicationForm.vue';

export default {
  title: 'Application Examples/Complete Mood Ecosystem',
  parameters: {
    layout: 'fullscreen',
    docs: {
      description: {
        component: 'Complete demonstration of the integrated mood tracking ecosystem with all components working together in a real-world loan application scenario.',
      },
    },
  },
};

export const CompleteLoanApplicationDemo = () => ({
  components: { 
    MoodTracker, 
  ...
  setup() {
    ...
    const moodHistory = ref([
      { emotion: 'optimistic', step: 1, timestamp: Date.now() - 600000, confidence: 0.89 },
      { emotion: 'focused', step: 2, timestamp: Date.now() - 300000, confidence: 0.92 },
      { emotion: 'confused', step: 3, timestamp: Date.now() - 100000, confidence: 0.78 }
    ]);
    ...
    const handleEmotionChange = (emotion) => {
      ...
      systemEvents.value.unshift({
        ...
        data: { emotion: currentEmotion.value, step: currentStep.value },
    ...
    const handleStepChange = (step) => {
      ...
      systemEvents.value.unshift({
        type: 'step_changed',
        data: { from: currentStep.value, to: step },
        timestamp: new Date().toLocaleTimeString()
      });
    };
    ...
    const simulateUserJourney = async (journeyType) => {
      const journeys = {
        confident: [
          { emotion: 'optimistic', step: 1, delay: 1000 },
          { emotion: 'focused', step: 2, delay: 2000 },
          { emotion: 'confident', step: 3, delay: 1500 },
          { emotion: 'excited', step: 4, delay: 1000 },
          { emotion: 'satisfied', step: 5, delay: 500 }
        ],
        struggling: [
          { emotion: 'optimistic', step: 1, delay: 1000 },
          { emotion: 'confused', step: 2, delay: 3000 },
          { emotion: 'frustrated', step: 3, delay: 4000 },
          { emotion: 'anxious', step: 3, delay: 2000 },
          { emotion: 'relieved', step: 4, delay: 2000 }
        ],
        abandonment: [
          { emotion: 'neutral', step: 1, delay: 1500 },
          { emotion: 'confused', step: 2, delay: 3000 },
          { emotion: 'frustrated', step: 3, delay: 5000 },
          { emotion: 'overwhelmed', step: 3, delay: 3000 }
        ]
      };
      ...
      for (const point of journey) {
        ...
        handleEmotionChange({ emotion: point.emotion, confidence: 0.85 });
      }
    };
...
export const TechnicalArchitectureOverview = () => ({
  template: `
