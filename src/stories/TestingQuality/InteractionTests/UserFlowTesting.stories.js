...
export default {
  title: 'Testing & Quality/Interaction Tests/User Flow Testing',
  parameters: {
    layout: 'fullscreen',
    docs: {
      description: {
        component: 'Interactive user flow testing scenarios for loan applications with step-by-step validation and error handling.',
      },
    },
  },
  argTypes: {
    flowType: {
      control: { type: 'select' },
      options: ['complete-application', 'validation-errors', 'loading-states', 'recovery-flow'],
      description: 'Type of user flow to test',
      defaultValue: 'complete-application',
    },
    simulateErrors: {
      control: { type: 'boolean' },
      description: 'Simulate validation errors for testing',
      defaultValue: false,
    },
    testLoadingStates: {
      control: { type: 'boolean' },
      description: 'Test loading and processing states',
      defaultValue: true,
    },
  },
};

export const CompleteApplicationFlow = (args) => ({
  ...
  methods: {
    ...
    isStepComplete() {
      switch (this.currentStep) {
        ...
        case 2:
          return this.formData.annualIncome && this.formData.monthlyDebt && 
                 this.formData.housingPayment;
        case 3:
...
CompleteApplicationFlow.args = {
  flowType: 'complete-application',
